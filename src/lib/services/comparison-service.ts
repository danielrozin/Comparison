...
function transformToPageData(
  ...
  return {
    ...
    metadata: {
      ...
      // DAN-1152: clamp the shortAnswer fallback at a word boundary (with "…")
      // instead of a raw 155-char mid-word cut. The old slice(0,155) produced a
      // string < the 160 limit, so the page-level clampDescription() was a no-op
      // and the mid-word truncation reached prod <meta>/og:description verbatim.
      metaDescription:
        row.metaDescription || clampDescription(row.shortAnswer) || "",
