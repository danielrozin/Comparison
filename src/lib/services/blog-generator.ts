...
export async function saveBlogArticle(
  ...
  // DAN-520: compute the title embedding once (null when no provider is
  // configured). Reused by both the semantic dedup gate and the row write so
  // we never spend two embedding calls per save.
  const titleEmbedding = await embedText(article.title);

  // DAN-520: refuse near-duplicate publishes (slug-prefix collision, topic
  // signature match, word-cosine, or semantic embedding cosine) before the DB
  // write. Rejections logged to Redis.
  if (!options.skipDedup) {
    const dedup = await checkBlogDedup(article.title, article.slug, {
      category: article.category,
      proposedEmbedding: titleEmbedding,
    });
    if (!dedup.ok) {
