...
export async function GET() {
  ...
  lines.push("- `Dataset.interactionStatistic: InteractionCounter(ReadAction, viewCount)` REMOVED from all comparison Dataset nodes — `view_count` is seed data, not analytics, so it was reporting engagement we cannot substantiate. Study rankings now use comparison centrality (count of published comparisons an entity appears in) instead of view counts (DAN-2037)");
  ...
  lines.push("- `/site-map` WebPage schema upgraded from minimal stub to full GEO/AEO stack: `abstract`, `alternativeHeadline`, `genre: \"Site Index\"`, `creativeWorkStatus`, `interactivityType`, `datePublished`/`dateCreated`/`dateModified`/`contentReferenceTime`, `thumbnailUrl` + `image` (1200×630 ImageObject with contentUrl/creditText/creator/copyrightHolder), `keywords`, `educationalLevel`, `teaches`, `educationalUse: \"reference\"`, `license`/`usageInfo`/`copyrightNotice`/`copyrightHolder`/`acquireLicensePage`, `audience`, `speakable`, `significantLink`, `potentialAction: [ReadAction, SearchAction]`; BreadcrumbList gets `@id`; metadata gains OG + Twitter image, DC.subject/rights/coverage/description, abstract, Twitter card labels (HB362)");
  lines.push("- `/about` schema gains `genre: \"About\"`, `contentReferenceTime`, `thumbnailUrl`, `image` (1200×630 ImageObject with contentUrl), `potentialAction: [ReadAction, SearchAction]`, `significantLink` array (trending, methodology, contact, site-map), and expanded `speakable` cssSelector covering mission + vision headings (HB363)");
  lines.push("- `/who-is-this-for` schema gains `genre: \"Guide\"`, `contentReferenceTime`, `datePublished`, `dateModified`, `thumbnailUrl`, `image` (1200×630 ImageObject), and upgraded `potentialAction` from single ReadAction to [ReadAction, SearchAction] array (HB363)");
  lines.push("- `/contact`, `/terms`, `/disclaimer`, `/privacy` schemas upgraded: `genre` (\"Contact\"/\"Legal\"), `abstract`, `alternativeHeadline`, `contentReferenceTime`, `thumbnailUrl`, `image` (1200×630 ImageObject with contentUrl) added to all four pages; `/contact` `potentialAction` upgraded to [ReadAction, CommunicateAction] with typed recipient Organization; OG+Twitter images now populated on all four pages (previously missing) (HB364)");
  lines.push("- `entitySchemaType()` in `src/lib/seo/schema.ts` extended: `automotive` → `Vehicle`, `car` → `Car`, `health` → `MedicalEntity`, `finance` → `FinancialProduct`, `sport` → `SportsActivity`, `education` → `EducationalOrganization`, `entertainment` → `CreativeWork`, `gaming` → `VideoGame` — previously all fell back to `Thing`; affects entity nodes in Knowledge Graph API, comparison schemas, and hub pages (HB363)");
  lines.push("- `/how-we-write-verdicts`, `/authors/daniel-rozin`, `/blog`, `/llm-comparisons` — BreadcrumbList JSON-LD added to all four pages (had visible breadcrumb UI but no structured data; AI crawlers now get typed ListItem+WebPage nodes for all navigation paths); `/authors/daniel-rozin` Person schema gains `sameAs: twitter.com/aversusb` (HB365)");
  lines.push("- `/search` WebPage node strengthened: added `speakable: SpeakableSpecification` (cssSelector: [\"h1\", \".faq-answer\"]) directly on the WebPage node (previously only on FAQPage child); added `publisher` Organization node referencing `/#organization` anchor; added `BreadcrumbList` @graph node (Home→Search); added `abstract`, `alternativeHeadline`, `isAccessibleForFree`, `creativeWorkStatus`, `datePublished`/`dateModified`; image node enriched with `creditText`, `creator`, `copyrightHolder`; extracted `SITE_URL`/`SITE_NAME`/`TODAY` constants (HB366)");
  lines.push("- `/tools/roi-calculator`, `/tools/loan-payoff-calculator`, `/tools/days-between-dates` — all three interactive calculator pages upgraded with full AEO/GEO schema: `WebPage` node added to `@graph` with `speakable` (cssSelector: h1, h2, dl dt, dl dd + XPath fallbacks), `abstract`, `genre` (\"Financial Calculator\"/\"Date Calculator\"), `interactivityType: \"active\"`, `contentReferenceTime`, `dateModified`, `isAccessibleForFree`; `WebApplication` node gains `potentialAction: [UseAction]` so AI agents (ChatGPT Actions, Copilot, Perplexity) can surface the calculator as a tool for user queries; all three URLs added to sitemap.ts (were previously uncrawled) (DAN-1578)");
