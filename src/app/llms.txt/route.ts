...
export async function GET() {
  ...
  lines.push("- `webPageSchema()` gains `contentReferenceTime` (derived from `dateModified ?? datePublished`) — GEO/AEO signal emitted on WebPage node for all pages using the shared helper; previously only Article nodes had this field; LLMs now receive time-qualified freshness on both WebPage and Article nodes for /compare/* and all content pages (DAN-2011)");
  lines.push("- Speakable CSS selectors on /compare/* pages verified against rendered HTML: `h1`, `#hero-tldr` (ComparisonHero), `#short-answer` (QuickAnswerTLDR/ShortAnswerBlock), `#verdict` (VerdictCard/Verdict), `#key-differences` (KeyDifferences), `#key-facts`, `#comparison-table`, `#expert-analysis` (ExpertAnalysis), `#faq` (FAQ) — all selectors match real DOM IDs; no mismatches found (DAN-2011)");
  lines.push("- Blog Article nodes gain `relatedLink[]` pointing to relatedComparisonSlugs — complements `significantLink` (WebPage-level) with CreativeWork-level graph edges so AI crawlers traverse the full topic cluster from the Article node directly; previously blog nodes had significantLink only (HB-DAN1578)");
  lines.push("- Hub page CollectionPage nodes gain `relatedLink[]` (up to 12 spoke comparisons) alongside `significantLink` (top-6 only) — broader AI crawler coverage for hub topic cluster discovery; hub relatedLink mirrors the same pattern already on compare + alternatives + blog Article nodes (HB-DAN1578)");
