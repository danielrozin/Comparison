...
> DAN-2547 identity**, diverging from this repo. Corrected 2026-07-20. If a scheduled
> send aborts on an identity mismatch, **check that file first** — the repo can be right
> and the send still abort.
>
> ⛔ **`.env.bak-dan2145-20260720` is NOT a restore point — never `cp` it back.** It is a
> snapshot of the *broken* pre-correction state, and it is wrong on BOTH fields:
> `RESEND_FROM_EMAIL="A Versus B <hello@aversusb.net>"` (the one domain the key 403s on)
> and `RESEND_API_KEY=re_Fd1z17…` (rotated out 2026-07-13). Restoring it guarantees a
> zero-send abort. Verified by diff 2026-07-20 13:56Z. It is kept only as evidence of
> what DAN-2547 wrote. The good values are the table below.
>
> **Verified state 2026-07-20 13:56Z** — `.env` mtime `12:35:42Z`, carrying
> `hello@aversusb-mail.com` + key `re_MmBBZs…`, i.e. the correct locked identity, and it
> has held since the 12:35Z correction. Re-check mtime at fire time: this file is
> machine-generated and has already been rewritten under us once today.
